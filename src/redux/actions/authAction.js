...
export const sendMagicLink = (email) => async (dispatch) => {
  try {
    const response = await useInsertData(`/api/v1/auth/sendMagicLink`, {
      email,
    });
    return response;
  } catch (e) {
...
//verify magic link
export const verifyMagicLink = (token, email) => async (dispatch) => {
  try {
    const response = await useInsertData(`/api/v1/auth/verifyMagicLink`, {
      token,
      email,
    });
    return response;
